...
A Task runs automatically when McLium starts.
Unlike a SubCommand, it does not require CLI arguments and does not wait for user input.

To create a Task, your `Main` class must inherit from `McLiumTaskModule`.

### Example

```python
from mclium.api import McLiumTaskModule


class Main(McLiumTaskModule):

    def create_task(self):
        """
        This method is called automatically when McLium starts.

        Place your task logic inside this method.
        """
...
When McLium loads plugins:

1. It reads `plugin.yml`
2. It imports the class defined in `main`
3. If the class inherits from `McLiumTaskModule`
4. It automatically calls:
...
* You want something to run automatically at startup
* You need background initialization logic
* You want to perform setup or environment checks
* You want to preload data or register services

Do not use a Task if the feature should only run when explicitly requested from the CLI. In that case, use `SubCommandModule`.
...
When McLium starts, the output will automatically appear.
...
If the class inherits from `McLiumTaskModule`, it will behave as a Task.
